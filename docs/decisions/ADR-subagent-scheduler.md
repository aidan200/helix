...
> 活规则锚：AD-7（调度编排）、AD-8（双通道异步交付）、AD-10（队列不落盘）、AD-3/TR-AD-24（模型三级解析链）、O-5（closure 双产物裁决）、O-6（kill 信号序列）、O-7（子进程启动形态）、TR-AD-4（装配同构）。
...
- **模型三级解析单点（AD-3/TR-AD-24）**：① profile.model（声明即最高）→ ② uiModelSlot（resource_state kind 槽位 UI 化）→ ③ spawn 会话快照 → ④ 全局兜底 getter（auth 分层 AD-2：key/模型源读 auth.json/default_model 现值，换 key 后下一请求/新子进程跟随）。高档有值即短路；返回完整 Model 对象全链透传（单点解析红线）。
