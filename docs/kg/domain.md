...
## 规则
①职责 = 根据 task 定义完成任务并给出结果（自动重试/生命周期控制/过程与结果查询）；结果后处理（落盘/呈现/修正）是任务类型的域逻辑，任务系统零产出处置概念（TR-AD-60）。②人对运行中任务只有生命周期控制（暂停/继续/取消），内容零干预；任务页零创建，创建按任务类型各有宿主。③确认只在开启前一次（任务内容卡或对话），执行全程无 gate。④阶段是通用结构：创建时定义、确认后冻结、落数据行不落代码；任务间无关系结构。⑤项目关联 = projects[] 0..n 普通标签，基数由任务类型 paramsSchema 声明。⑥批次产出落 kg 经 KgWriteService 唯一入口（含 batchCreateNodes 批量 op），衔接面 = taskId/origin_batchId/layer 三元数据——机械注入已全 op 覆盖（2026-08-30 修复：createNode 组合 declareAnchors op 未注入的丢章根因修复，6 类写入路径 change_log 全盖 task_id；iterationId 改服务端机械解析：workspace 当前迭代 → 目标库最近迭代锚 → 结构化报错不猜，LLM 传参降级为可选覆盖）；批次重跑幂等 = 旧产出 supersede + 新跑（TR-AD-64）。⑦daemon 重启扫 running 任务重开编排会话续跑，断点粒度 = stage/batch 行。⑧任务删除（人工）：仅终态可删、清任务域全部记录（含批次实例 plan）、不动 kg 产出。⑨重置面（2026-08-30 新增）：kg.graph.purge 清空本项目图谱（全量清表 + 索引态复位；运行中 kg-bootstrap 任务存在时拒绝——防 done 任务悬挂引用；两步确认），清空后 bootstrap 准入恢复 eligible 可重新发起；kg.index.delete 删除索引（.codegraph 目录 + 状态复位 absent + 联动停 fs-watch watcher，重建后经 onSynced 自动重挂）——两命令职责严格分层（.helix-kg vs .codegraph），全重置 = 两者都做。⑩任务页呈现面（2026-08-30 裁决集）：批次列表 = 全量批次按阶段分组（原只取当前阶段批次——done 任务只见末阶段批次的过滤缺陷修复；TaskBatchDto 带 stageSeq 分组键）；insertBatch 的 seq 由 WriteQueue 单写线程原子赋予（原引擎侧 count+1 在并发工具调用下竞态——首跑实证同阶段批次 seq 全 1）；详情头叙述句（currentNarrative）全链移除；「任务结果」tab（原「结果查询」改名）只呈现阶段文字报告。
