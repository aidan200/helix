...
| `--text-dim` | `#718198`（见 T4.2 调值注） | `#64748B`（slate-500） | 说明 / 辅助（输入 placeholder、hud-label） |
| `--text-dim-rgb` | `113 129 152` | `100 116 139` | 暗列随 T4.2 调值同步（通道同步铁律） |
...
生成关系：dim 亮列 slate-500、暗列 `#718198`（T4.2 AA 调值后双主题分档，见下注）；faint = 各主题 muted → dim 方向 70% 处的 sRGB 插值。同一套 slate 色阶明暗互用，切换主题 = 明度换挡。
...
**T4.2 a11y 调值（CL-6 兑现，2026-08-18）**：暗列 `--text-dim` 原 `#64748B`（slate-500，L = 0.171）对暗色三底仅 **dim/bg 4.06 / dim/elevated 3.84 / dim/panel 3.96**，低于 WCAG AA 4.5:1。新值暗列 `#718198` = slate-500 → slate-400（暗 muted）方向 28% sRGB 插值（113 129 152），L = 0.215：对 `--bg #0a0e16` = **4.87:1** ✓、对 `--bg-elevated #0f1521` = **4.61:1** ✓、对 `--panel-solid #0B1120` = **4.75:1** ✓（对 `--void` 亦 5.02:1）。
- 双主题同值不可行的裁决依据：暗列 elevated 底达标要求 L ≥ 0.209，亮列白底达标要求 L ≤ 0.183（#64748B 对白底 4.76:1 已在窗内），两亮度窗无交集——单值双主题同达标数学上不可能，故暗列独调、亮列保留 `#64748B` 不回退（沿 F4.2 faint 先例：AA 硬约束下双主题同值让位于分档）。
- 达标修正非风格变更（Q-5 delta = 无）：不换色相（同 slate 灰蓝族插值）、不动四档文字层级结构、不引入新 token；调值后暗列 dim（L 0.215）仍略深于 faint（L 0.218），维持 F4.2 注记的交叉关系。
- 断言：`scripts/perf-a11y-audit.mjs`（`bun run audit:a11y`）色表随本注册表校准，复跑全过 = CL-6 验收锚。
