...
export const zhCN = {
  chat: {
    ...
    settings: {
      ...
      skills: {
        title: "用户技能",
        subtitle: "技能目录 ~/.helix/skills：放入 <技能名>/SKILL.md（frontmatter 带 name 与 description）即自动装载；启用/禁用在「智能体」页按 agent 维护",
        loading: "加载中…",
        empty: "暂无用户技能——在 ~/.helix/skills/<技能名>/ 下创建 SKILL.md（frontmatter 带 name 与 description）即自动装载",
        kindMain: "主会话",
        kindSub: "子代理",
        enabled: "启用",
        disabled: "已禁用",
        ...
        fieldDescription: "一句话描述（进 agent 技能清单）",
        fieldBody: "正文（Markdown）",
        fieldBodyFull: "SKILL.md 全文（含 frontmatter，可编辑）",
        bodyPlaceholder: "技能正文：何时用、怎么做、硬性规则…",
        bodyFullPlaceholder: "---\nname: my-skill\ndescription: 一句话描述\n---\n\n技能正文…",
        importLabel: "选择 SKILL.md 文件（原文导入，可在下方编辑）",
        ...
        createFail: {
          ...
          "already-exists": "同名技能已存在（~/.helix/skills 下冲突）",
          "bad-frontmatter": "SKILL.md 需以 --- frontmatter 块开头（含 name 与 description）",
        },
      },
      mcp: {
        title: "MCP Server",
        subtitle: "标准 MCP 接入：声明 server → 动态发现工具 → 自动接入 agent 工具面（重启后保持）",
        importLabel: "导入配置",
    importFail: "导入失败：无法解析为 MCP 配置（需 mcpServers 块或单 server 对象，且含 command 字段）",
    importMulti: "发现 {count} 个 server——已填入首个「导入后逐个重复导入即可」",
    addServer: "新增 Server",
        fieldName: "名称（工具命名空间前缀）",
        fieldCommand: "启动命令",
        fieldArgs: "参数（空格分隔）",
        fieldEnabled: "启用（关闭 = 保留配置但不连接）",
        fieldDeferred: "懒加载（工具经 discover 按需装载）",
        fieldDeferredHint: "开启后该 server 的工具不进入初始工具集，agent 调用 `${server}__discover` 时按需装载（省 token）；关闭则全部工具随会话常驻",
        deferredBadge: "懒加载",
        addConfirm: "添加",
        adding: "添加中…",
        test: "测试连接",
        ...
        testFail: "连接失败",
        formInvalid: "名称与命令必填",
        connectFailed: "连接失败（配置已保留，可重试）",
        addFailed: "添加失败",
        loading: "加载中…",
        empty: "暂无 MCP server——新增一个开始接入（如 shadcn / magicui 官方 MCP）",
        delete: "删除",
        deleteConfirm: "确认删除",
        disabledNote: "已停用",
        toolCount: "{count} 个工具",
      ...
      general: {
        ...
        groupScheduling: "SubAgent 调度",
        groupPort: "服务端口",
        maxConcurrent: "并发上限（maxConcurrent）",
        maxQueued: "队列上限（maxQueued）",
        wsPort: "WS 监听端口",
        schedNote: "保存后下一次调度判定即生效，无需重启",
        portNote: "修改后下次启动生效；本次运行端口不变",
        portArgvOverride: "本次运行由 --port 参数覆盖为 {port}，修改存储值不影响本次",
        groupSandbox: "命令沙箱",
        sandbox: "沙箱开关",
        sandboxOn: "开启",
        sandboxOff: "关闭",
        sandboxNote: "对新会话与新 SubAgent 任务生效；运行中的会话保持原状态。开启后 bash 与文件写入被限制在工作空间与 ~/.helix 内，自检失败自动降级为关闭",
    ...
    modelsConfig: {
      defaultLabel: "全局兜底模型",
      thinkingDefaultLabel: "全局默认推理强度",
      thinkingDefaultClear: "清除（未配置 = 各智能体默认关）",
      thinkingUnsupported: "当前全局兜底模型不支持推理强度档位",
      refresh: "刷新目录",
      ...
      sendFailToast: "未连接 daemon，操作未发出",
      writeFailToast: "操作失败 · {message}",
    },
    tsFormat: "HH:mm",
  },
  trace: {
  ...
  agents: {
    ...
    skillAudienceTask: "任务 SOP",
    skillAudienceTaskHint: "任务类型 SOP：经任务系统 kickoff 全文注入，agent 面恒禁用（不可开启）",
