...
describe("P-2 会话侧栏", () => {
  it("活跃会话态：无草稿卡；清单卡片按序渲染（标题/运行态/时间档位）+ connected 触发清单拉取", () => {
    ...
    topologyRef.current = {
      ...
      list: LIST,
      modelConfig: createInitialModelConfigState(), agentConfig: { revision: 0 },
      webStatus: null,
    };
    ui();
  ...
  it("草稿态（sessionId=null + connected）：草稿卡在顶 + 新建按钮走 newDraft", () => {
    ...
    topologyRef.current = { active: stateRef.current, background: {}, list: [], modelConfig: createInitialModelConfigState(), agentConfig: { revision: 0 }, webStatus: null };
    ui();
  ...
  it("删除二次确认：trash 进入 confirming（单值互斥——另一卡进入时前卡退出）；取消复原", () => {
    ...
    topologyRef.current = { active: stateRef.current, background: {}, list: LIST, modelConfig: createInitialModelConfigState(), agentConfig: { revision: 0 }, webStatus: null };
    ui();
