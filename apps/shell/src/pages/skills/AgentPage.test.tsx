...
/** 统一启停批：orchestrator 升格 profiles 第三块（真实 kind 可配置面）。 */
const ORCH_PROFILE_BLOCK: AgentConfigProfileBlock = {
  ...
  tools: [
    ...
    { name: "kg", enabled: true, snippet: "查询项目知识图谱（只读）" },
  ],
  // task 类技能行可见恒禁（audience-guard——builtin 源进内置组灰只读）
  skills: [
    {
      name: "code-review",
...
function feedList(mainOver: Partial<AgentConfigProfileBlock> = {}, subOver: Partial<AgentConfigProfileBlock> = {}): void {
  feed({
    ...
    payload: {
      profiles: [{ ...MAIN_BLOCK, ...mainOver }, { ...SUB_BLOCK, ...subOver }, ORCH_PROFILE_BLOCK],
      system: [KGW_BLOCK],
    },
  } as EventEnvelope);
}

/** master-detail 选中（agent-roster 批）：点左栏行。 */
function selectAgent(kind: string): void {
  fireEvent.click(document.querySelector(`[data-agent-row="${kind}"]`)!);
}
...
describe("智能体页组件（M6 T4）", () => {
  ...
  it("① 进页拉取 + AppLayout 壳 + master-detail：左栏两组分组/只读徽标；选中 main 后详情卡渲染", async () => {
    ...
    expect(roRows).toHaveLength(2); // 统一启停批：派生两块（kg-writer / reviewer；orchestrator 升格可配置）
