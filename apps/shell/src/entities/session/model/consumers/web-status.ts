 ...
 * - web.stop.result（停止写面回执）→ 拓扑原引用直通（停止后的 idle 状态
 *   回流经 web.status.changed 广播；applied 语义无 UI 消费面）。
 ...
 * （两帧 payload 同形状，契约 §16.8）；stop.result → 拓扑原引用（状态回流
 * 经广播，见文件头注）。
 */
export function applyWebEvent(topo: TopologyState, frame: EventEnvelope): TopologyState {
  ...
  return topo; // web.stop.result：直通（状态回流经 web.status.changed 广播）
}
