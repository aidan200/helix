...
export function validateTaskParams(
  ...
  // 静默吞掉未知键会让拼错字段名的调用方误以为生效）。Object.hasOwn 只认
  // 自有键——`k in schema` 走原型链会让 toString/__proto__ 等成员名逃过校验。
  const unknownKeys = Object.keys(params).filter((k) => !Object.hasOwn(manifest.paramsSchema, k));
