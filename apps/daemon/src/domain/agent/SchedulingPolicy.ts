...
/**
 * SchedulingPolicy —— SubAgent 调度策略（architecture.md §4.1，C-7/AD-7 整包）。
 *
 ...
 * - isStalled：idle 超阈值判定（警示不自动杀；阈值构造可注入小值——K1）。
 *
...
export interface SchedulingPolicyOptions {
  ...
  /** stalled 判定阈值 ms；缺省 300_000（5min）。测试可注入小值（K1）。 */
...
export class SchedulingPolicy {
   ...
   * ③ 运行位满且队列满 → reject（错误回 LLM，不抛并发错误——F-3②）。
   */
