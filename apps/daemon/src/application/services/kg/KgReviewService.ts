 ...
 * 非空恰是评审对象——**允许反复发起**，准入从简 = 索引存在即可（building
 * 中索引已存在也放行：sync 只动符号层，评审读知识层零冲突）。未建索引 →
 * kg.review.not_eligible（message 带原因 index_absent）。
 *
...
/** 准入复核结论（create 前置；reason = 契约词表 index_absent——从简单因）。 */
export type ReviewEligibility = { readonly eligible: true } | { readonly eligible: false; readonly reason: "index_absent" };
...
export type KgReviewErrorCode = "kg.review.not_eligible" | "task.validation_failed" | "task.type_unknown" | "KG_E_PARAM";
    ...
    if (!eligibility.eligible) {
      return {
        ...
        error: {
          ...
          message: "index_absent：项目尚未构建索引（先完成一次机械构建，B1 冷启动链）——体检面向存量图谱，无索引无评审对象",
        },
      };
    }
