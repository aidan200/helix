...
export class EventStream implements EventPublisherPort {
  ...
  broadcastAgentConfigChanged(payload: {
    profileKind: "main-session" | "subagent-worker" | "task-worker" | "orchestrator" | "subagent-kg-writer" | "subagent-code-reviewer"; // R7 扩四值；D5 第五值；任务 subAgent 独立配置批扩 task-worker（第六值）
