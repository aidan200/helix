/**
 * session 族命令处理（AD-1：case 体自 WsServerAdapter.routeCommand 机械迁出）。
 *
 ...
 * 等机械代换，不改分支/字符串/回执时序。
 *
 * v0.2/v0.3 session 族（契约 B §1）：list/loadHistory/delete 全局
 ...
 * （session.not_found）。冷会话经注册表懒加载后即为合法目标。
 */
...
export function handleSessionSubscribe(ctx: SessionCommandContext): void {
  ...
  if (!sender) return;
  // v0.3（契约 §2.1）：payload.tier 可选档位——缺省 full（既有语义
  ...
  // v0.2：per-session 订阅——信封 sessionId 指定目标会话；
  ...
  void resolveTargetSession(ctx).then((target) => {
    ...
    // 热修：agentState/model 取目标会话 runtime（随视图同源组装），
...
export function handleSessionUnsubscribe(ctx: SessionCommandContext): void {
  const sender = ctx.ws.data.sender;
  // 定稿：对称 per-session 退订——与 subscribe 同一目标会话解析规则
  void resolveTargetSession(ctx).then((target) => {
...
export function handleSessionDelete(ctx: SessionCommandContext): void {
  ...
  void ctx.directory
    ...
    .catch((err) => {
      ...
      // 判别改 err.code 码匹配（原 err.name 三元链；无 code 旧对象
