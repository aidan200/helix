...
export class CdpConnectionManager implements BrowserPort {
  ...
  async setFilesInTab(tabId: string, selector: string, files: readonly string[]): Promise<void> {
    await this.connect();
    const sid = await this.ensureSession(tabId);
  ...
  async scrollTab(tabId: string, y = 3000, direction: ScrollDirection = "down"): Promise<void> {
    await this.connect();
    const sid = await this.ensureSession(tabId);
    let js: string;
    if (direction === "top") {
    ...
    await this.sendCDP("Runtime.evaluate", { expression: js, returnByValue: true }, sid);
    this.registry.touch(tabId);
