/**
 * rg 路径三级解析单点（AD-2/F3.1，architecture §4.4）——framework-free
 * 纯函数：零 env/fs 直接依赖，全部环境面由入参注入（可单测）。
 *
 * 固定三级顺序：① 包内 bundle（壳经 env HELIX_RG_PATH 注入，由装配层
 * 读出后传入）→ ② 用户配置（config.json rgPath，经 paths.ts/config.ts
 * 读出后传入）→ ③ 宿主 PATH 探测（注入的 PATH 字符串按 `:` 分段，逐段
 * 拼 `<dir>/rg` 探测）。
 *
 * 决策消解（T1.1 brief）：
