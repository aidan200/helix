...
import { ensureSchemaEvolved, migrateLegacyDefaultModel } from "./schema-evolution";
import { MAIN_AGENT_KIND, type WriteJob, type WriteQueueOptions } from "./write-jobs";
 ...
 * 清理面的写语句也宿主本文件（AG-06：任务四表 DML 唯一宿主，O-1 表分域
 * 断言钉死）——语句工厂 prepareWorkLedgerStatements + 直连连接工厂
 * openTaskLedgerDatabase 导出供 WorkLedger 装配（类在 WorkLedger.ts，
 * 零写 SQL）。跨进程串行化 = WAL + busy_timeout（父/子连接各自设置；
 * 子连接不能依赖父进程设置）。
 *
 * 体量治理拆分：WriteJob 类型面在 write-jobs.ts（零 SQL 数据声明）；
 * 守护式 schema 演进在 schema-evolution.ts（会话域 DDL/迁移宿主，
 * AG-06 白名单成员）；任务四表 DML 与全部任务域级联仍宿主本文件。
 */
