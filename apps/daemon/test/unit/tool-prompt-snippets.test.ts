 ...
 * pi 工具符号封装边界不变）。main 11 工具 + subagent 7 工具（共享 7 + 编排 3
 * + 动态族单 browser）全覆盖；中文一句话（单行——进 system prompt 的扁平清单行，多行破坏清单格式）。
 */

describe("ToolPromptSnippets 注册表（M6 T2）", () => {
  test("① 两 profile 全集全覆盖：main 11 名 + subagent 7 名均有非空 snippet", () => {
