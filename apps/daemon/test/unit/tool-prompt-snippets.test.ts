 ...
 * pi 工具符号封装边界不变）。main 11 工具 + subagent 8 工具（共享 7 + browser
 * （H-3 转发接入）+ 编排 3 仅 main）全覆盖；中文一句话（单行——进 system prompt 的扁平清单行，多行破坏清单格式）。
 */

describe("ToolPromptSnippets 注册表（M6 T2）", () => {
  test("① 两 profile 全集全覆盖：main 11 名 + subagent 8 名均有非空 snippet", () => {
    ...
    // 全集形状锚定（ResourceService toolsCatalog 同源）：main 11 / subagent 8（H-3 +browser）
    expect(MainSessionProfile.tools).toHaveLength(11);
    expect(SubAgentProfile.tools).toHaveLength(8);
  });

  test("② snippet 为中文一句话：非空、单行（无换行符）", () => {
    // 恰 11 条（main 全集；subagent 共享其中 8 条——单一注册表不分 kind）
