 ...
 * pi 工具符号封装边界不变）。main 8 工具 + subagent 5 工具（共享 5 + 编排 3）
 * 全覆盖；中文一句话（单行——进 system prompt 的扁平清单行，多行破坏清单格式）。
 */

describe("ToolPromptSnippets 注册表（M6 T2）", () => {
  test("① 两 profile 全集全覆盖：main 8 名 + subagent 5 名均有非空 snippet", () => {
    ...
    // 全集形状锚定（ResourceService toolsCatalog 同源）：main 8 / subagent 5
    expect(MainSessionProfile.tools).toHaveLength(8);
    expect(SubAgentProfile.tools).toHaveLength(5);
  });

  test("② snippet 为中文一句话：非空、单行（无换行符）", () => {
    // 恰 8 条（main 全集；subagent 共享其中 5 条——单一注册表不分 kind）
    expect(Object.keys(TOOL_PROMPT_SNIPPETS).sort()).toEqual(
      ["agent_send", "agent_spawn", "agent_status", "bash", "edit", "grep", "read", "write"].sort(),
    );
