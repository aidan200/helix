 ...
 * pi 工具符号封装边界不变）。main 10 工具 + subagent 7 工具（共享 7 + 编排 3）
...
describe("ToolPromptSnippets 注册表（M6 T2）", () => {
  test("① 两 profile 全集全覆盖：main 8 名 + subagent 5 名均有非空 snippet", () => {
    ...
    // 全集形状锚定（ResourceService toolsCatalog 同源）：main 10 / subagent 7
    expect(MainSessionProfile.tools).toHaveLength(10);
    expect(SubAgentProfile.tools).toHaveLength(7);
  });

  test("② snippet 为中文一句话：非空、单行（无换行符）", () => {
    // 恰 10 条（main 全集；subagent 共享其中 7 条——单一注册表不分 kind）
    expect(Object.keys(TOOL_PROMPT_SNIPPETS).sort()).toEqual(
      ["agent_send", "agent_spawn", "agent_status", "bash", "edit", "grep", "read", "web_fetch", "web_search", "write"].sort(),
    );
