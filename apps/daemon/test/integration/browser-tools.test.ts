...
describe("CoreToolExecutor 条件注册（options.browser 先例 = orchestration）", () => {
  ...
  test("② 有 browser：单名注册；main 全集 22 名一次装配成功", () => {
    ...
    expect(MainSessionProfile.tools).toHaveLength(22); // T3.3 kg 双工具 + T2.4 task_create（AD-7）+ D3 task_report + W1-B codegraph + ⑤ 链 C park/resume 双工具（P1 仅 main）+ main-session plan 批 plan 三名（两域同构）
