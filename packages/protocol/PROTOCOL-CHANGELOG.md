# Helix WS 协议变更登记（PROTOCOL-CHANGELOG）

> 本文件 = PROTOCOL.md 的批次演进备案：所有「批次登记 / additive 微批备案」
> 性质内容（原 §10–§14 v0.1–v0.4 演进登记、§17.5–§17.11 v0.5–v0.11 批次
> 登记、§18–§23 v0.11 后 additive 微批）自 PROTOCOL.md 迁入，**原节号原样
> 保留**（未重排）——仓内「PROTOCOL-CHANGELOG.md §N」引用即指本文件中原
> 节号。
>
> 现行契约（§1–§9 / §15 / §16 / §17.1–§17.4）与 SoT 守护见 PROTOCOL.md；
> 登记纪律：版本 bump / additive 微批 = 本文件加批次节（或批内补登节）+
> PROTOCOL.md §15/§16 落字段行，同 commit（PROTOCOL.md §17.2①）。
>
> 迁移批次：protocol-split（设计：docs/temp/protocol-split-design.md）——
> 以下内容逐字节保真迁入（hash 校验），零内容改写。
...
目录，daemon 下发侧单点消费）：chat（v0 chat 族 10，含 engine.error 热修 + engine.retrying 网络重试批 + error entry 批 error.entry）/
