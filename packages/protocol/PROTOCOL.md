...
| `agent.config.read_only` | agent.config.set_enabled 对系统三 kind（orchestrator / subagent-kg-writer / subagent-code-reviewer）tool/skill/mcp-server 启停的写面拒绝（编排归位批：orchestrator 归位系统 kind——仅槽位型可写；前端只读只是表现，后端拒绝才是事实） | 发 error 帧，**连接保持** |
...
| 字段 | 类型 | 可选性 | 登记版本 | 语义 |
|---|---|---|---|---|
| `profileKind` | `"main-session" \| "subagent-worker"` | 可选 | v0.6 | 目标 kind：缺省 = 全部可配置 kind（双块，main-session/subagent-worker 序固定；task-worker 已撤——任务派生 worker 回归 subagent-worker 同 kind 同配置面）；携带 = 单块 |
...
只读系统 kind 写面拒绝（编排归位批后口径）：profileKind 携带系统三 kind
（`"orchestrator"` / `"subagent-kg-writer"` / `"subagent-code-reviewer"`）
且 resourceType 为
tool/skill/mcp-server → `connection.error { code:
"agent.config.read_only" }`（连接保持）——orchestrator = 系统机制 kind
（变相禁用：加载链同构但 kind 缺省全禁 + 写面只读）；kg-writer/reviewer
工具集/技能面派生 worker，硬层拒绝不依赖前端表现；三 kind 的
model/thinking 槽位型均放行；其余未知 kind 仍 `command.invalid_payload`。mcp-server
...
| 字段 | 类型 | 可选性 | 登记版本 | 语义 |
|---|---|---|---|---|
| `profileKind` | `"main-session" \| "subagent-worker" \| "orchestrator" \| "subagent-kg-writer" \| "subagent-code-reviewer"` | 必填 | v0.6 | 目标 kind（可编辑双 kind 全型可写；系统三 kind 仅槽位型——task-worker 已撤） |
...
设置 SubAgent 调度预算（SQLite KV 单键 JSON；运行期可调——下一次预算
判定生效，无需重启）。路由：全局命令。结果帧：
`config.set_scheduling.result`（点对点，§16.6）。

| 字段 | 类型 | 可选性 | 登记版本 | 语义 |
|---|---|---|---|---|
| `maxConcurrent` | `number` | 必填 | config 瘦身批 | 运行中 SubAgent 上限（≥1 整数；缺省 3） |
| `maxQueued` | `number` | 必填 | config 瘦身批 | FIFO 队列上限（≥0 整数；缺省 8） |

#### `config.get_scheduling`

查询调度预算。路由：全局命令。结果帧：`config.get_scheduling.result`
（点对点，§16.6）。
...
设置 WS 监听端口（SQLite KV `daemon_port` 键；**下次启动生效**——port 是
启动期定格参数。运行优先级：argv `--port` > KV > 缺省 7333）。路由：全局
命令。结果帧：`config.set_port.result`（点对点，§16.6）。

| 字段 | 类型 | 可选性 | 登记版本 | 语义 |
|---|---|---|---|---|
| `port` | `number` | 必填 | config 瘦身批 | 端口号（0-65535；0 = 随机） |

#### `config.get_port`

查询端口配置。路由：全局命令。结果帧：`config.get_port.result`（点对点，
§16.6；含 argv 覆盖信息）。
...
| 字段 | 类型 | 可选性 | 登记版本 | 语义 |
|---|---|---|---|---|
| `maxConcurrent` | `number` | 必填 | config 瘦身批 | 当前生效运行中 SubAgent 上限 |
| `maxQueued` | `number` | 必填 | config 瘦身批 | 当前生效 FIFO 队列上限 |

#### `config.set_scheduling.result`

| 字段 | 类型 | 可选性 | 登记版本 | 语义 |
|---|---|---|---|---|
| `maxConcurrent` | `number` | 必填 | config 瘦身批 | 写后的运行中上限 |
| `maxQueued` | `number` | 必填 | config 瘦身批 | 写后的队列上限 |

#### `config.get_port.result`

| 字段 | 类型 | 可选性 | 登记版本 | 语义 |
|---|---|---|---|---|
| `effectivePort` | `number` | 必填 | config 瘦身批 | 本次运行实际监听端口（argv 覆盖时 = argv 值） |
| `storedPort` | `number \| null` | 必填 | config 瘦身批 | 存储端口（KV daemon_port；null = 未设置用缺省 7333） |
| `overriddenByArgv` | `boolean` | 必填 | config 瘦身批 | 本次运行是否经 argv --port 显式覆盖 |
