...
describe("TP-CL2-② 导出面（index.ts 汇总）", () => {
  ...
  test("② 常量语义值 + 目录计数（task 批：命令 45 / 事件 58；v0.11 版本位保持）", () => {
    ...
    expect(protocol.EVENT_TYPES.length).toBe(74); // kg.candidates.list 批：+1（kg.candidates.list.result；config 批后 73）
    expect(Object.keys(protocol.EVENT_CHANNELS).length).toBe(74); // 登记目录恰等（kg.candidates.list 批 +1）
  });
});
