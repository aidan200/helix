...
describe("TP-CL2-② 导出面（index.ts 汇总）", () => {
  ...
  test("② 常量语义值 + 目录计数（task 批：命令 45 / 事件 58；v0.11 版本位保持）", () => {
    ...
    expect(protocol.COMMAND_TYPES.length).toBe(57); // config 批：+2（config.set_compaction/config.get_compaction；R7 后 55）
    expect(protocol.EVENT_TYPES.length).toBe(73); // config 批：+2（config.*.result；R7 后 71）
    expect(Object.keys(protocol.EVENT_CHANNELS).length).toBe(73); // 登记目录恰等（config 批 +2）
  });
});
