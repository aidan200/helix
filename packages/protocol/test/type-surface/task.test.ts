...
describe("task 批（T1.5）：命令/事件/通道登记", () => {
  ...
  test("事件目录：task.changed 唯一新事件登记（+1；结果帧不入目录）", () => {
    ...
    expect(EVENT_TYPES.length).toBe(68); // kg.health 批 +1 ∧ kg 评审批 +1 ∧ 网络重试批 +1（engine.retrying，插 chat 族不占末位）后当前值
  ...
  test("通道归属：task.changed 挂既有 notification 通道（不新增 Channel 值，契约 §0）", () => {
    ...
    expect(Object.keys(EVENT_CHANNELS).length).toBe(68); // 网络重试批 +1
  });
