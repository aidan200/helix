...
describe("task 批（T1.5）：命令/事件/通道登记", () => {
  test("命令目录：十命令登记且排序在 workspace 族之后", () => {
    ...
    // diff 批（diff.get）登记在 task 族之后（本目录追加惯例：新批尾段）
    expect(COMMAND_TYPES.slice(-TASK_COMMANDS.length - 1)).toEqual([...TASK_COMMANDS, "diff.get"]);
    expect(COMMAND_TYPES.length).toBe(63); // diff 批 +1 后当前值
  });
  ...
  test("事件目录：task.changed 唯一新事件登记（+1；结果帧不入目录）", () => {
    ...
    // diff 批（diff.changed）登记在 task.changed 之后（本目录追加惯例）
    expect(EVENT_TYPES[EVENT_TYPES.length - 1]).toBe("diff.changed");
    expect(EVENT_TYPES.length).toBe(80); // diff 批 +1 后当前值
  ...
  test("通道归属：task.changed 挂既有 notification 通道（不新增 Channel 值，契约 §0）", () => {
    ...
    expect(Object.keys(EVENT_CHANNELS).length).toBe(80); // diff 批 +1（diff.changed，挂既有 session 通道）
  });
