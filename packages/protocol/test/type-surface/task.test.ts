...
describe("task 批（T1.5）：命令/事件/通道登记", () => {
  test("命令目录：九命令登记且排序在 workspace 族之后", () => {
    ...
    expect(COMMAND_TYPES.length).toBe(57); // R7 全局兜底批 +1（model.set_thinking_default）后当前值
  });
  ...
  test("事件目录：task.changed 唯一新事件登记（+1；结果帧不入目录）", () => {
    ...
    expect(EVENT_TYPES.length).toBe(73); // R7 全局兜底批 +1（model.set_thinking_default.result，插 model 族不占末位）后当前值
