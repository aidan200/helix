...
// 帧版本位取值域（v0.6：0 = v0/v0.1 历史帧兼容读，"0.6" = 当前批帧）
type _VIsVersion = Expect<Equal<HelloCommand["v"], FrameVersion>>;

type _FrameVersionDomain = Expect<Equal<FrameVersion, 0 | "0.6">>;

// hello 协商位严格 "0.6" 单值（不取 FrameVersion 联合；fail-fast）
type _HelloVersion = Expect<Equal<HelloPayload["protocolVersion"], "0.6">>;
...
// ④ 版本位批次标记："0.6"（typeof PROTOCOL_VERSION 单值；FrameVersion / hello 联动见上）
type _ProtocolVersionV03 = Expect<Equal<typeof PROTOCOL_VERSION, "0.6">>;
...
describe("envelope：信封分型/版本位/兼容红线/预留与路由位（源 TP-CL2-①② / TP-v0.1-①③ / TP-v0.2-① / TP-v0.3-②）", () => {
  ...
  test("PROTOCOL_VERSION = \"0.6\"；当前批帧 v 位全为 \"0.6\"，历史帧 v=0 合法（兼容读）", () => {
    expect(PROTOCOL_VERSION).toBe("0.6");
    for (const frame of [...v02Events, ...v02ResultEvents, ...v02Commands, ...v03Commands, ...v03Events, helloFrame]) {
      expect(frame.v).toBe("0.6");
    }
  ...
  test("PROTOCOL_VERSION = \"0.6\"；hello 协商位单值联动；当前批帧 v 位全 \"0.6\"", () => {
    expect(PROTOCOL_VERSION).toBe("0.6");
    expect(helloFrame.payload.protocolVersion).toBe("0.6");
