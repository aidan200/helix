...
// 帧版本位取值域（v0.9：0 = v0/v0.1 历史帧兼容读，"0.9" = 当前批帧）
type _VIsVersion = Expect<Equal<HelloCommand["v"], FrameVersion>>;

type _FrameVersionDomain = Expect<Equal<FrameVersion, 0 | "0.9">>;

// hello 协商位严格 "0.9" 单值（不取 FrameVersion 联合；fail-fast）
type _HelloVersion = Expect<Equal<HelloPayload["protocolVersion"], "0.9">>;
...
// ④ 版本位批次标记："0.9"（typeof PROTOCOL_VERSION 单值；FrameVersion / hello 联动见上）
type _ProtocolVersionV03 = Expect<Equal<typeof PROTOCOL_VERSION, "0.9">>;
