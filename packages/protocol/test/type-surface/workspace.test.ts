...
describe("workspace 批（W1）：命令/事件/通道登记", () => {
  test("命令目录：workspace.get / workspace.open 登记且排序在 kg 族之后（kg-bootstrap 批前尾段）", () => {
    ...
    // kg-bootstrap 批（T3.2）五命令 + kg 维护批（C1）两命令 + kg 评审批（W2-F）一命令 + kg.candidates.list 批一命令插在 kg 族尾（kg.candidates.list 为 kg 族末位）
    const kgEnd = COMMAND_TYPES.findIndex((t) => t === "kg.candidates.list");
