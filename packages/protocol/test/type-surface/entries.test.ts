...
// 负向断言：v 位不接受目录外版本（0/"0.12" 之外）
// @ts-expect-error v 位必须是 FrameVersion（0 | "0.12"）
const badVersion: HelloCommand = { v: 1, type: "hello", payload: { token: "t", protocolVersion: "0.12" } };

// 负向断言（v0.2 起保持）：hello 协商位不接受 v0 数值（严格 "0.12" 单值）
// @ts-expect-error protocolVersion 必须是 "0.12"
const badHelloLegacy: HelloCommand = { v: "0.12", type: "hello", payload: { token: "t", protocolVersion: 0 } };
...
describe("entries：EntryDto 判别式联合与快照 additive + 通道族承载与负向回读（源 TP-CL2-④ / TP-v0.1-①② / TP-v0.2-①③ / TP-v0.3-①）", () => {
  ...
  test("switch(entry.kind) 四分支窄化：steerState 仅 message 变体", () => {
    ...
    expect(badVersion.payload.protocolVersion).toBe("0.12");
