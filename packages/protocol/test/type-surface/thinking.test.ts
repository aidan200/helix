...
describe("v0.11：thinking 批 additive（T1.1，AD-2/AD-4）", () => {
  test("目录登记：thinking.set ∈ COMMAND_TYPES；thinking.changed ∈ EVENT_TYPES/EVENT_CHANNELS（thinking 族）", () => {
    ...
    // 计数：27 → 28 命令 / 47 → 48 事件（thinking 批历史口径；当前常量含后续 kg 批 +6/+6、workspace 批 +2/+3、task 批 +9/+1 → 45/58）
    expect(COMMAND_TYPES.length).toBe(50);
    expect(EVENT_TYPES.length).toBe(63);
  ...
  test("v0.11 sot：§17.11 批次登记节与 envelope 版本位 + 四块登记一致", () => {
    ...
    expect(COMMAND_TYPES.length).toBe(50); // kg 批 +6、workspace 批（W1）+2、task 批 +9、kg-bootstrap 批 +5 后当前值
    expect(EVENT_TYPES.length).toBe(63); // kg 批 +6、workspace 批 +3、task 批 +1、kg-bootstrap 批 +5 后当前值
