...
describe("v0.11：thinking 批 additive（T1.1，AD-2/AD-4）", () => {
  test("目录登记：thinking.set ∈ COMMAND_TYPES；thinking.changed ∈ EVENT_TYPES/EVENT_CHANNELS（thinking 族）", () => {
    ...
    expect(COMMAND_TYPES.length).toBe(55); // R7 全局兜底批 +1 后当前值
    expect(EVENT_TYPES.length).toBe(71); // R7 全局兜底批 +1（model.set_thinking_default.result）
    expect(Object.keys(EVENT_CHANNELS).length).toBe(71); // R7 全局兜底批 +1
  });
  ...
  test("v0.11 sot：§17.11 批次登记节与 envelope 版本位 + 四块登记一致", () => {
    ...
    expect(COMMAND_TYPES.length).toBe(55); // R7 全局兜底批 +1 后当前值 // kg 批 +6、workspace 批（W1）+2、task 批 +9、kg-bootstrap 批 +5、kg 维护批 +2、kg.health 批 +1、kg 评审批 +1 后当前值
    expect(EVENT_TYPES.length).toBe(71); // kg 批 +6、workspace 批 +3、task 批 +1、kg-bootstrap 批 +5、kg 维护批 +2、kg.health 批 +1、kg 评审批 +1、网络重试批 +1 后当前值
