...
describe("agent.config 事件族 payload（v0.6）", () => {
  ...
  // ── additive 微批（agent-roster）：只读系统块 + 写面拒绝错误码 ──
  ...
  test("system 只读块（编排归位批）：system 三块（orchestrator 归位 + 派生双块）+ profiles 双块（task-worker 已撤）", () => {
    ...
    // 新形态：双可编辑块（main/sub）+ 系统三只读块（orchestrator 在前）
