 ...
 *    model.catalog 刷新全部失败（快失不超时）→ P-3 菜单列出 builtin 合并
 *    目录（anthropic 分组在场）+ SQLite 默认读面（claude-sonnet-4-5 DEFAULT
 *    徽标 = model.get_default）；
 ...
 *    0600 权限位 + Credential JSON 形状 + 锁文件零残留 + P-4 脱敏回显。
 *
...
test.describe("T4.2 CL-3 模型真链路（真 daemon）", () => {
  test("builtin fallback 无外网 + set_model 下一 turn 生效（in-flight 不变）+ auth.json 写入面", async ({ e2e, page }) => {
    ...
    // 死代理下目录刷新全部失败 → 保底 builtin（合并行为：分组在场）
    ...
    // ── ⑤ auth.json 写入面（auth.set_key → AuthStore）────────
    ...
    writeEvidence(
      ...
      "txt",
      [
        "T4.2 CL-3 模型真链路：PASS",
        "① builtin fallback 无外网（K-1）: 死代理下 model.catalog 刷新全失败 → builtin 合并目录（anthropic 分组）",
        `   + SQLite 默认读面（${DEFAULT_INIT} DEFAULT 徽标 = model.get_default）`,
