...
test.describe("CL-7 smoke（mock mode on）", () => {
  test("P-1 页面打开，app 壳渲染（header/composer），mock 建连到 connected", async ({ mock, page }) => {
    ...
    // hello 首帧契约（握手）：token + protocolVersion（v0.12 bump 机械跟随——coord.changed 批
    // T1.1 版本位 v0.12；契约 = PROTOCOL.md §2/§17.12）
    const hello = await mock.waitForCommand("hello");
    expect(hello.payload).toEqual({ token: "e2e-dev-token", protocolVersion: "0.12" });
